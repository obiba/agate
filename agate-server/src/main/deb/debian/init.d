...
get_running() 
{
    return `ps -U $AGATE_USER --no-headers -f | egrep -e '(java|daemon)' | grep -c . `
}

get_running_daemon() 
{
    return `ps -U $AGATE_USER --no-headers -f | egrep -e '(daemon)' | grep -c . `
}
...
force_stop() 
{
    get_running
    if [ $? -ne 0 ]; then 
        killall -u $AGATE_USER java || return 3
    fi
}
...
do_start()
{
    ...
    $DAEMON $DAEMON_ARGS -- $JAVA $JAVA_ARGS -cp $CLASSPATH -DAGATE_HOME=$AGATE_HOME -DAGATE_DIST=$AGATE_DIST -DAGATE_LOG=$AGATE_LOG $MAIN_CLASS $AGATE_ARGS || return 2
}
...
#
# Make sure agate tmp dir exists, otherwise daemon calls will fail
#
if [ ! -d $TMPDIR ]; then 
  mkdir $TMPDIR
  chown -R $AGATE_USER:adm $TMPDIR
  chmod -R 750 $TMPDIR
fi
      ...
      case "$?" in 
         ...
         *) 
              ...
              if [ $procs -eq 0 ]; then 
                  ...
                  echo "An instance of agate is running at the moment"
                  echo "but the pidfile $PIDFILE is missing"
              else 
                  echo "$procs instances of agate are running at the moment"
                  echo "but the pidfile $PIDFILE is missing"
              fi
              ;;
      esac
