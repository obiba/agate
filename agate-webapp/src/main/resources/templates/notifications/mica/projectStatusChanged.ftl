  ...
  <div style="margin: auto;max-width: 700px;">
    <div
      ...
      <div
        ...
        <h3 style="font-family: &quot;Lato&quot;,&quot;Helvetica Neue&quot;,Helvetica,Arial,sans-serif;font-weight: 400;line-height: 1.1;color: inherit;margin-top: 0;margin-bottom: 0;font-size: 17px;" th:text="#{mica.email.projectStatusChanged.title(${organization})}">
          <span>(organization)</span> - Project
        </h3>
      </div>
      <div class="panel-body" style="padding: 15px;">
        ...
        <div th:switch="${status}" style="margin: 0 0 10px;">
          <p th:case="DRAFT" th:utext="#{mica.email.projectStatusChanged.body.draft(${documentId})}">
            The project <strong>(document id)</strong> is ready to be edited.
          </p>
          <p th:case="UNDER_REVIEW" th:utext="#{mica.email.projectStatusChanged.body.underReview(${documentId})}">
            The project <strong>(document id)</strong> is ready to be reviewed. You can now approve or reject its publication.
          </p>
          <p th:case="'DELETED'" th:utext="#{mica.email.projectStatusChanged.body.deleted(${documentId})}">
            The project <strong>(document id)</strong> was marked as being deleted. You can now delete it permanently.
          </p>
          <p th:case="*" th:utext="#{mica.email.projectStatusChanged.body.other(${documentId}, ${status})}">
            The project <strong>(document id)</strong> status was changed to <strong>(status)</strong>.
          </p>
        </div>
        <p style="margin: 0 0 10px;">
        </p><p style="margin: auto;text-align: center;">
        <a href="" th:href="@{${publicUrl} + '/#/' + ${documentType} + '/' + ${documentId}}" target="_blank"
          style="color: #ffffff;text-decoration: none;display: inline-block;margin-bottom: 0;font-weight: normal;text-align: center;vertical-align: middle;-ms-touch-action: manipulation;touch-action: manipulation;cursor: pointer;background: #2c3e50 none;white-space: nowrap;padding: 10px 15px;font-size: 15px;line-height: 1.5;border-radius: 4px;-webkit-user-select: none;-moz-user-select: none;-ms-user-select: none;user-select: none;border: 1px solid #2c3e50;" th:text="#{mica.email.projectStatusChanged.link}">View Project</a>
      </p>
      </div>
    </div>
