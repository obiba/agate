    ...
    <div
      ...
      <div
        ...
        <h3 style="font-family: 'Lato','Helvetica Neue',Helvetica,Arial,sans-serif;font-weight: 400;line-height: 1.1;color: inherit;margin-top: 0;margin-bottom: 0;font-size: 17px;">
          ${msg("mica.email.networkStatusChanged.title", organization)}
        </h3>
      </div>
      <div class="panel-body" style="padding: 15px;">
        ...
        <p style="margin: 0 0 10px;">
          <#switch status>
            <#case "DRAFT">
              ${msg("mica.email.networkStatusChanged.body.draft", documentId)}
              <#break>
            <#case "UNDER_REVIEW">
              ${msg("mica.email.networkStatusChanged.body.underReview", documentId)}
              <#break>
            <#case "DELETED">
              ${msg("mica.email.networkStatusChanged.body.deleted", documentId)}
              <#break>
            <#default>
              ${msg("mica.email.networkStatusChanged.body.other", documentId, status)}
          </#switch>
        </p>
        <p style="margin: 0 0 10px;">
        </p><p style="margin: auto;text-align: center;">
          <a href="${publicUrl}/admin#/${documentType}/${documentId}" target="_blank"
             style="color: #ffffff;text-decoration: none;display: inline-block;margin-bottom: 0;font-weight: normal;text-align: center;vertical-align: middle;-ms-touch-action: manipulation;touch-action: manipulation;cursor: pointer;background: #2c3e50 none;white-space: nowrap;padding: 10px 15px;font-size: 15px;line-height: 1.5;border-radius: 4px;-webkit-user-select: none;-moz-user-select: none;-ms-user-select: none;user-select: none;border: 1px solid #2c3e50;">${msg("mica.email.networkStatusChanged.link")}</a>
        </p>
      </div>
    </div>
